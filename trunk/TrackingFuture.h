...
enum Model3DPointState {  NewPointState, InModelState,InModelNotUseState, FitErrorState, NotInModelState, OutLintersState, 
						NotLocalizedState,OutLintersOFMoveState,OutLintersKeyFrameMoveState,
						OutLintersModel3DShiftJump,OutLintersModel3DShift,
						OutLintersEyeFuter};
enum FuterLocationSourceState {  FirstEstymationOFState, SecondEstymationOFState,EyeCorrectState, Model3dState};
...
public:
	int m_Model3DCounter;
		...
		FuterLocationSourceState m_FuterLocationSource;
		//float m_Model3DAvrFitErr;
	...
	//	m_ImgDescriptor =  cvCreateImage( cvSize(FutureImgDescriptorSize,FutureImgDescriptorSize), 8, 1 );
		...
		m_bIsEyeCorner = false;
		//m_bEyeFuter = false;
		m_ShiftToModel3dProjectLocation = cvPoint2D32f(0,0);
		m_ShiftToModel3dProjectLocationPrev = cvPoint2D32f(0,0);
		m_Model3DState = InModelState;
		m_EyeFuterFactor = 1.0;
...
public:
	...
	bool IsOutLiter()
	{
	return (!(  (m_Model3DState != OutLintersState) &&
		(m_Model3DState != OutLintersOFMoveState) &&
		(m_Model3DState != OutLintersKeyFrameMoveState) &&
	(m_Model3DState != OutLintersModel3DShiftJump) &&
		(m_Model3DState != OutLintersModel3DShift) &&

		(m_Model3DState != OutLintersEyeFuter)) ||(m_Model3DState == NotLocalizedState));
	}



//--------------eye futer data-----------
		CvPoint2D32f m_EyeCenterShift;

//--------------eye futer data-----------



		float m_EyeFuterFactor;
...
// jesli zostanie usunienty z listy cech jest ustawiane
	...
	CvPoint2D32f m_Model3dProjectLocation;
	CvPoint2D32f m_ShiftToModel3dProjectLocation;
	CvPoint2D32f m_ShiftToModel3dProjectLocationPrev;
